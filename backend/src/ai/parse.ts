...
import { recordClaudeSuccess } from "./health.js";
...
export async function parseExpense(
  ...
  const staticBlock: CachedTextBlock = {
    type: "text",
    text:
  ...
  const systemBlocks: (CachedTextBlock | PlainTextBlock)[] = [staticBlock];
  ...
  }).catch((err) => console.error("[usage] Failed to log parseExpense:", err));
...
export async function classifyMessage(
      ...
      client.messages.create({
        ...
        tools: [
          ...
          },
          {
            name: "query_spend",
            description:
              "Retrieve spending data when the user asks a question about their expenses",
            input_schema: {
              type: "object" as const,
              properties: {
                category: {
                ...
                time_range_label: {
                  type: "string",
                  description:
                    "Human-readable label (e.g. 'this month', 'last week', 'this year')",
                },
                start_date: {
                  type: "string",
          ...
          },
          {
            ...
            description:
              "Ask the user to clarify a spending query when details are ambiguous",
            input_schema: {
              type: "object" as const,
              properties: {
  ...
  const usage = response.usage as ExtendedUsage;
  void insertClaudeUsage({
    intent: "classifyMessage",
    input_tokens: usage.input_tokens,
    output_tokens: usage.output_tokens,
    cache_read_tokens: usage.cache_read_input_tokens ?? 0,
    cache_creation_tokens: usage.cache_creation_input_tokens ?? 0,
    model: response.model,
  }).catch((err) => console.error("[usage] Failed to log classifyMessage:", err));
