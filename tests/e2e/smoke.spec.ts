...
test('manage workspace renders categories, budgets, tags, and statements', async ({ page }) => {
  ...
  await expect(page.getByText('Failed captures')).toBeVisible()
  await expect(page.getByText('Capture failure mix')).toBeVisible()
  await expect(page.getByText('Latest restore detail')).toBeVisible()
  ...
  await expect(page.locator('.badge-passed').filter({ hasText: 'passed' })).toBeVisible()
  ...
  await expect(page.getByRole('button', { name: /expense update expense/ })).toBeVisible()
})
