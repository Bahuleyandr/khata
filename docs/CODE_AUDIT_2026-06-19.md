...
- **Only outstanding item:** **M5** stays dropped (ENABLE-ALWAYS would break `pg_restore` — the real fix is the non-owner DB role, an ops/go-live task). Optional future nicety: M15 could move CBC+HMAC → a native AEAD (`age`/GPG) if those tools are added to the backup image.
