...
- **Still deferred (with rationale):** **M4** FK (`expenses.user_id → ledgers`) — defense-in-depth for an invariant that already holds by construction, and even a `NOT VALID` FK rejects the migration-smoke's `user_id=999` insert and would need test-fixture churn; **M9** bot-liveness watchdog — the poller already fails-fast on crash (exit → k8s restart); detecting a *silent* wedge reliably needs `@grammyjs/runner`, a larger change; **M13 infra** (non-root Postgres/MinIO, default-deny NetworkPolicy) — can't be runtime-tested against the HELD deploy and risk volume-perm/CNI breakage, so left as operator-verify tasks; **M15 cipher** (CBC → authenticated) — needs `age`/`gpg` tooling not in the aws-cli image.
