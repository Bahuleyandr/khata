...
- **Wave 3 (PR 4) — branch `fix/audit-2026-06-19-pr4`, backend verify 301/301:** **M6** migration-checksum drift guard, **L8** amount_cents `CHECK > 0` (migration 033, `NOT VALID`), **L1** rate-limit before media download/transcribe, **M13** nginx security headers (frame-ancestors/nosniff/Referrer-Policy), **M14** NodePort/host-firewall posture documented, **M15** offsite-backup CronJobs fail loud on missing crypto. Also migrated the integration vitest config off the deprecated `poolOptions` (#5a follow-up).
- **Dropped:** **M5** ENABLE-ALWAYS triggers — would break `pg_restore` of closed-month rows and overwrite `updated_at` on restore, for ~no real protection (a superuser/table-owner can disable triggers regardless); the real fix for the owner-can-disable concern is the non-owner DB role (an ops task).
- **Wave 4 (PR 5) — branch `fix/audit-2026-06-19-pr5`, backend verify 305/305, migration-smoke green:** **M4** FK `expenses.user_id → ledgers` (migration 034, `NOT VALID`; smoke seeds its synthetic ledgers 995–999; no `paid_by_user_id` FK since a payer can be a since-removed member per H3); **M9** bot-liveness getMe heartbeat + `/live` endpoint wired to the k8s *liveness* probe (readiness stays `/health`); **M13 infra** non-root Postgres/MinIO (fsGroup pattern) + dropped caps + a data-tier NetworkPolicy; **M15 cipher** offsite-backup encrypt-then-HMAC integrity (both CronJobs). The deploy manifest changes are YAML-valid but **applied/verified by the operator on un-hold** (deploy HELD; non-root needs a fresh/chowned PVC, NetworkPolicy enforcement is CNI-dependent).
- **Wave 5 (PR 6) — branch `fix/audit-2026-06-19-pr6`, backend verify 305/305, migration-smoke green:** **M5 real fix** — a non-owner least-privilege `khata_app` role (migration 035, DML + sequence grants + default privileges) that the app now connects as. Its writes still fire the immutability triggers, but it **cannot disable triggers / DDL / TRUNCATE** — migration-smoke asserts exactly that via `SET ROLE`. `migrate.ts` provisions the role's LOGIN password from `APP_DB_PASSWORD` (never in a migration; trust-auth CI skips it); migrations keep running as the owner. The discarded `ENABLE ALWAYS` approach stays dropped. **Operator on un-hold:** add `APP_DB_PASSWORD` to the Secret and run migrate (which provisions `khata_app`) *before* the backend connects — see the runbook.
- **No remediation items remain.** Optional future nicety: M15 CBC+HMAC → a native AEAD (`age`/GPG) if those tools are added to the backup image.
